...
int main(int argc, char **argv)
{
	...
	mem = (char*) numa_alloc_onnode(page_size, node);
	...
	for (i = 0; i < nr_pages; i++) {
		...
		char	*new_mem = (char*) numa_realloc(mem, (i+1)*page_size, (i+2)*page_size);
		if (!new_mem) {
	...
	mem = (char*) numa_realloc(mem, (nr_pages + 1)*page_size, page_size);
	if (!mem) {
		perror("numa_realloc() failed");
		exit(1);
	}
